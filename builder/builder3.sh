...
git_prompt_metadata() {
    ...
    CURRENT_BUILD_LABEL=$(ui_inputbox "Build Label" \
        "Enter a custom label for this build (used in firmware file naming):" "$default_label")
...
# Copy binaries into 'firmware releases/' and push git repository
git_release_workflow() {
    ...
    ui_wait_screen "Firmware Release" "Archiving binaries to firmware releases/…"

    # Target filenames with label and timestamp
    local release_prefix="${CFG_BOARD}_${CURRENT_BUILD_LABEL}_${ts}"
    local rel_boot="${RELEASES_DIR}/${release_prefix}_bootloader.bin"
    local rel_part="${RELEASES_DIR}/${release_prefix}_partition-table.bin"
    local rel_mpy="${RELEASES_DIR}/${release_prefix}_micropython.bin"

    # Also make a copy with generic names in the release directory if desired
    cp -v "$FW_BOOTLOADER" "$rel_boot" >> "$log_file" 2>&1 || true
    cp -v "$FW_PARTITION" "$rel_part" >> "$log_file" 2>&1 || true
    cp -v "$FW_MICROPYTHON" "$rel_mpy" >> "$log_file" 2>&1 || true

    log_write "$log_file" INFO "Firmware binaries copied to: $RELEASES_DIR"

    # Check git repository
    if ! command -v git >/dev/null 2>&1 || [[ ! -d "$REPO/.git" ]]; then
        ui_msgbox "Git Notice" "Git is not initialized in $REPO. Binaries saved to 'firmware releases'."
        return 0
    fi
    ...
    if [[ "$CFG_AUTO_GIT_PUSH" == "yes" ]]; then
        ...
        local push_rc=0
        git push >> "$log_file" 2>&1 || push_rc=$?

        if [[ $push_rc -eq 0 ]]; then
            log_write "$log_file" INFO "Git push succeeded."
            ui_msgbox "Release Complete" \
                "✔ Firmware built & released!\n\n1. Binaries copied to:\n   firmware releases/${release_prefix}_*.bin\n\n2. Git commit:\n   '$CURRENT_COMMIT_MSG'\n\n3. Pushed successfully to remote!"
        else
        ...
        ui_msgbox "Release Archived" \
            "Binaries copied to 'firmware releases/' and committed locally.\n(Git auto-push disabled in Settings)"
    fi
}
...
build_info_ui() {
    ...
    lines+=("  Releases Folder : $RELEASES_DIR")
    lines+=("")
