...
#   §15  Log viewer
#   §16  Clean build
#   §17  Full automated workflow
#   §18  Settings menu
#   §19  Firmware Identity & Branding
#   §20  Partition Manager
#   §21  Build Automation Menu
#   §22  Main menu + entry point
# =============================================================================
...
readonly FW_MERGED="$BUILD_DIR/firmware-merged.bin"
readonly BUILD_SDKCONFIG="$BUILD_DIR/sdkconfig"

# ── Flash offsets (ESP32-S3, standard MicroPython layout) ───────────────────
readonly OFFSET_BOOTLOADER="0x0"
readonly OFFSET_PARTITION="0x8000"
readonly OFFSET_APP="0x10000"
...
SELECTED_PORT=""
SETTINGS_LOADED=0
CURRENT_RELEASE_NAME=""     # e.g. "jelly-jammy_20260825_141230"
CURRENT_RELEASE_DIR=""      # full path to the release folder in use
CURRENT_RELEASE_IS_NEW=1    # 1 = brand new release, 0 = reusing an existing one
CURRENT_COMMIT_MSG=""
...
_slugify() {
    local input="$1"
    local out
    out=$(echo "$input" | tr '[:upper:]' '[:lower:]')
    out=$(echo "$out" | tr -s ' _' '-')
    out=$(echo "$out" | tr -cd '[:alnum:]-')
    out=$(echo "$out" | sed -e 's/^-*//' -e 's/-*$//' -e 's/-\{2,\}/-/g')
    echo "$out"
}
...
release_select_or_new() {
    local -a menu_items=()
    local -a rows=()
    mapfile -t rows < <(_release_list)

    menu_items+=("__NEW__" "★ New release…")
    ...
    if [[ "$choice" == "__NEW__" ]]; then
        local codename
        codename=$(ui_inputbox "New Release" \
            "Enter a codename for this release (e.g. \"jelly jammy\"):\n\nThe date will be appended automatically." \
            "")
        [[ -z "$codename" ]] && { ui_msgbox "Release" "A codename is required — cancelled."; return 1; }

        local slug; slug=$(_slugify "$codename")
        [[ -z "$slug" ]] && slug="release"

        local ts; ts=$(date '+%Y%m%d_%H%M%S')
        CURRENT_RELEASE_NAME="${slug}_${ts}"
        CURRENT_RELEASE_DIR="$RELEASES_DIR/$CURRENT_RELEASE_NAME"
        CURRENT_RELEASE_IS_NEW=1
    else
        CURRENT_RELEASE_DIR="$choice"
        CURRENT_RELEASE_NAME=$(basename "$choice")
        CURRENT_RELEASE_IS_NEW=0
    fi
...
build_merged_image() {
    ...
    if [[ ! -f "$FW_BOOTLOADER" || ! -f "$FW_PARTITION" || ! -f "$FW_MICROPYTHON" ]]; then
        log_write "$log_file" ERROR "Cannot merge — one or more source binaries missing"
        return 1
    fi

    log_section "$log_file" "Merging firmware image"
    log_write "$log_file" CMD "esptool --chip esp32s3 merge_bin -> $FW_MERGED"
    ...
    python3 -m esptool \
        ...
        --flash_freq 80m \
        -o "$FW_MERGED" \
        "$OFFSET_BOOTLOADER" "$FW_BOOTLOADER" \
        "$OFFSET_PARTITION"  "$FW_PARTITION" \
        "$OFFSET_APP"        "$FW_MICROPYTHON" \
        >> "$log_file" 2>&1

    local rc=$?
    if [[ $rc -eq 0 && -f "$FW_MERGED" ]]; then
        log_write "$log_file" INFO "Merged image created: $FW_MERGED"
        return 0
    else
        log_write "$log_file" ERROR "merge_bin failed (RC: $rc)"
        return 1
    fi
}
...
git_release_workflow() {
    ...
    [[ -z "$CURRENT_RELEASE_DIR" ]] && CURRENT_RELEASE_DIR="$RELEASES_DIR/release_${ts}"
    [[ -z "$CURRENT_RELEASE_NAME" ]] && CURRENT_RELEASE_NAME=$(basename "$CURRENT_RELEASE_DIR")
...

    {
        ...
        echo "# Started : $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
        echo "# Release : $CURRENT_RELEASE_NAME"
        echo "# Folder  : $CURRENT_RELEASE_DIR"
        echo "# Commit  : $CURRENT_COMMIT_MSG"
        echo "# ────────────────────────────────────────────"
    } > "$log_file"

    ui_wait_screen "Firmware Release" "Merging firmware image…"
    if ! build_merged_image "$log_file"; then
        ui_msgbox "Release Failed" "Could not merge firmware into a single flashable image.\n\nCheck log: $log_file"
        return 1
    fi

    local release_bin="$CURRENT_RELEASE_DIR/${CURRENT_RELEASE_NAME}.bin"

    ui_wait_screen "Firmware Release" "Copying flashable image to release folder…"
    cp -v "$FW_MERGED" "$release_bin" >> "$log_file" 2>&1

    log_write "$log_file" INFO "Flashable image copied to: $release_bin"

    # Check git repository
    if ! command -v git >/dev/null 2>&1 || [[ ! -d "$REPO/.git" ]]; then
        ui_msgbox "Git Notice" "Git is not initialized in $REPO.\n\nFlashable image saved at:\n$release_bin"
        return 0
    fi
...
build_firmware() {
    ...
    if [[ "$silent" == "0" ]]; then
        release_prompt_metadata || return 0
...
_flash_summary_screen() {
    ...
    local ref_file="$FW_MERGED"
    [[ "$mode" == "3image" ]] && ref_file="$FW_MICROPYTHON"
    [[ -f "$ref_file" ]] && \
        fw_ts=$(stat -c '%y' "$ref_file" 2>/dev/null | cut -d. -f1 || echo "unknown")
    ...
    lines+=("")
    if [[ "$mode" == "merged" ]]; then
        lines+=("  Flashing a single merged image:")
        lines+=("  0x00000  $(basename "$FW_MERGED")")
    else
        lines+=("  Flashing 3 separate images:")
...
_flash_validate() {
    ...
    ui_msgbox "Flash Error" "No firmware images found in:\n$BUILD_DIR\n\nPlease build first."
    return 1
}
...
flash_firmware() {
    ...
    _flash_summary_screen "$port" "$mode"
    ...
    if [[ "$mode" == "merged" ]]; then
        cmd+=( "$OFFSET_BOOTLOADER" "$FW_MERGED" )
    else
        cmd+=(
            "$OFFSET_BOOTLOADER" "$FW_BOOTLOADER"
            "$OFFSET_PARTITION"  "$FW_PARTITION"
...
build_info_ui() {
    ...
    [[ -f "$FW_MERGED" ]] && build_ts=$(stat -c '%y' "$FW_MERGED" 2>/dev/null | cut -d. -f1)

    local release_count=0
    [[ -d "$RELEASES_DIR" ]] && release_count=$(find "$RELEASES_DIR" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l)
    ...
    lines+=("  Releases Count  : $release_count")
    lines+=("")
    lines+=("  Board           : $CFG_BOARD")
    lines+=("  Build Dir       : $BUILD_DIR")
    lines+=("  Latest Merged   : $build_ts")
    lines+=("")
...
workflow_auto() {
    ...
    ui_msgbox "Automated Workflow" "Beginning complete workflow: Verify → Build → Merge → Release → Push → Flash"
...
_build_show_result() {
    ...
    if [[ "$rc" -eq 0 ]]; then
        ...
        ok+=("")
        ok+=("  Firmware images (build dir, not release folder):")
        ok+=("    bootloader.bin        $boot_sz")
        ok+=("    partition-table.bin   $pt_sz")
        ok+=("    micropython.bin       $fw_sz")
        ok+=("")
        ok+=("  A single merged, flashable .bin will be produced")
        ok+=("  next and copied into the release folder.")
        ok+=("")
        ok+=("  Log: $log_file")  ok+=("")
...
build_clean() {
    ...
    if ! ui_yesno "Clean Build" "Perform a CLEAN build for $CFG_BOARD?\n\nRelease: $CURRENT_RELEASE_NAME"; then
        return 0
    fi
...
build_incremental() {
    ...
    if ! ui_yesno "Incremental Build" "Perform an INCREMENTAL build?\n\nRelease: $CURRENT_RELEASE_NAME"; then
        return 0
    fi
...
build_automation_ui() {
    while true; do
        local -a items=(
            "CLEAN"       "Clean Build (rm build + make + merge + git release)"
            "INCREMENTAL" "Incremental Build (make + merge + git release)"
...
# =============================================================================
# §22  WELCOME SCREEN + MAIN MENU + ENTRY POINT
# =============================================================================
...
show_welcome() {
    ...
    msg+="  Branch     : $branch\n"
    msg+="  Board      : $CFG_BOARD\n"
    msg+="  Releases   : firmware_releases/\n"
