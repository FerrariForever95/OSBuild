...
# ── Board-dependent paths (NOT readonly — recomputed by _board_resolve_paths
# whenever the selected board changes; see §07b BOARD DEFINITIONS) ───────────
# ZENO_CORE and ZENO_FORGE are real, separate MicroPython board directories
# (ports/esp32/boards/ZENO_CORE, ports/esp32/boards/ZENO_FORGE) — there is no
# single hardcoded make target any more. BOARD is always kept in sync with
# CFG_BOARD_KEY; nothing else in this file should assume a fixed board name.
BOARD=""
BUILD_DIR=""
BOARD_DIR=""
MPCONFIGBOARD_H=""     # MICROPY_HW_BOARD_NAME etc.
SDKCONFIG_ZENO=""       # per-board flash/PSRAM sdkconfig fragment
FW_BOOTLOADER=""
FW_PARTITION=""
FW_MICROPYTHON=""
BUILD_SDKCONFIG=""     # generated after first build

# _board_resolve_paths — recompute every board-dependent path from CFG_BOARD_KEY.
# Called once at startup (end of cfg_load) and again whenever board_apply_selection
# changes CFG_BOARD_KEY, so BUILD_DIR/BOARD_DIR/FW_*/etc. can never drift out of
# sync with the board the user actually selected (see §07b).
_board_resolve_paths() {
    BOARD="$CFG_BOARD_KEY"
    BUILD_DIR="$PORT_DIR/build-${BOARD}"
    BOARD_DIR="$PORT_DIR/boards/${BOARD}"
    MPCONFIGBOARD_H="$BOARD_DIR/mpconfigboard.h"
    SDKCONFIG_ZENO="$BOARD_DIR/sdkconfig.zeno"
    FW_BOOTLOADER="$BUILD_DIR/bootloader/bootloader.bin"
    FW_PARTITION="$BUILD_DIR/partition_table/partition-table.bin"
    FW_MICROPYTHON="$BUILD_DIR/micropython.bin"
    BUILD_SDKCONFIG="$BUILD_DIR/sdkconfig"
}
...
# ── Multi-board partition tables (Board Selection, §07b) ────────────────────
readonly PARTITION_16MIB_ZENO="$PORT_DIR/partitions-16MiB-zeno.csv"
readonly PARTITION_8MIB_ZENO="$PORT_DIR/partitions-8MiB-zeno.csv"

# ── Default Settings (overridden by settings file) ──────────────────────────
DEFAULT_BOARD_KEY="ZENO_CORE"    # Board Selection key, see §07b BOARD_* maps
...
# ── ui_passwordbox TITLE MESSAGE  →  echoes entered text (masked input) ─────
ui_passwordbox() {
    local title="$1" msg="$2"
    local h=$(( UI_H * 40 / 100 )); [[ $h -lt 8 ]] && h=8
    local result
    case "$UI_BACKEND" in
        whiptail|dialog)
            result=$(_wt --title " $title " --passwordbox "$msg" "$h" "$UI_W")
            echo "$result"
            ;;
        text)
            echo -e "\n  ── $title ──\n$msg" | sed 's/^/  /'
...
cfg_load() {
    CFG_BOARD_KEY="$DEFAULT_BOARD_KEY"
    ...
    if [[ -f "$SETTINGS_FILE" ]]; then
        ...
        while IFS= read -r line || [[ -n "$line" ]]; do
            ...
            if [[ "$line" =~ ^([A-Za-z_]+)=(.*)$ ]]; then
                ...
                val="${BASH_REMATCH[2]}"
                case "$key" in
                    CFG_BOARD_KEY)      CFG_BOARD_KEY="$val" ;;
    ...
    # Guard against a stale/invalid board key (e.g. an older settings file
    # saved before boards were renamed, or manual edits) — fall back to the
    # default rather than silently operating on a non-existent board dir.
    if ! board_key_valid "$CFG_BOARD_KEY"; then
        CFG_BOARD_KEY="$DEFAULT_BOARD_KEY"
    fi
...
env_verify() {
    ...
    # sdkconfig.zeno
    if [[ -f "$SDKCONFIG_ZENO" ]]; then
        _chk "sdkconfig.zeno" "pass" ""
    else
        _chk "sdkconfig.zeno" "warn" "not found: $SDKCONFIG_ZENO"
    fi
...
# Check if sdkconfig.zeno matches the PSRAM mode of the SELECTED board
# (§07b Board Selection) — Octal for Zeno Core, Quad for Zeno Forge.
# Returns 0 = correct, 1 = problem found, 2 = file missing
board_config_check() {
    if [[ ! -f "$SDKCONFIG_ZENO" ]]; then
        return 2   # file missing
    fi

    local expected="${BOARD_PSRAM_MODE[${CFG_BOARD_KEY:-ZENO_CORE}]:-OCT}"  # OCT | QUAD
    local other="QUAD"; [[ "$expected" == "QUAD" ]] && other="OCT"

    local expected_ok=0 other_disabled=0

    grep -q "CONFIG_SPIRAM_MODE_${expected}=y" "$SDKCONFIG_ZENO" 2>/dev/null && expected_ok=1
    # The other mode should either be absent or explicitly set to n
    if ! grep -q "CONFIG_SPIRAM_MODE_${other}=y" "$SDKCONFIG_ZENO" 2>/dev/null; then
        other_disabled=1
    fi

    [[ $expected_ok -eq 1 && $other_disabled -eq 1 ]] && return 0 || return 1
}

# Repair sdkconfig.zeno to match the SELECTED board's PSRAM mode —
# called after user approval
board_config_repair() {
    local ts
    ts=$(date '+%Y%m%d_%H%M%S')
    local backup="${SDKCONFIG_ZENO}.backup_${ts}"

    local expected="${BOARD_PSRAM_MODE[${CFG_BOARD_KEY:-ZENO_CORE}]:-OCT}"  # OCT | QUAD
    local other="QUAD"; [[ "$expected" == "QUAD" ]] && other="OCT"

    log_section "$LOG_REPAIR" "Board Config Repair"
    log_write "$LOG_REPAIR" INFO "Repairing $SDKCONFIG_ZENO (board: ${CFG_BOARD_KEY:-ZENO_CORE}, expected PSRAM: $expected)"
    log_write "$LOG_REPAIR" INFO "Backup → $backup"

    # Create timestamped backup
    cp "$SDKCONFIG_ZENO" "$backup"

    # Apply corrections using sed — never rewrites the entire file
    # 1. Ensure CONFIG_SPIRAM_MODE_<expected>=y is present
    if grep -q "CONFIG_SPIRAM_MODE_${expected}" "$SDKCONFIG_ZENO"; then
        sed -i "s/^.*CONFIG_SPIRAM_MODE_${expected}.*\$/CONFIG_SPIRAM_MODE_${expected}=y/" \
            "$SDKCONFIG_ZENO"
    else
        echo "CONFIG_SPIRAM_MODE_${expected}=y" >> "$SDKCONFIG_ZENO"
    fi

    # 2. Disable CONFIG_SPIRAM_MODE_<other>
    if grep -q "CONFIG_SPIRAM_MODE_${other}=y" "$SDKCONFIG_ZENO"; then
        sed -i "s/^CONFIG_SPIRAM_MODE_${other}=y\$/# CONFIG_SPIRAM_MODE_${other} is not set/" \
            "$SDKCONFIG_ZENO"
    fi
...
board_config_ui() {
    local board_key="${CFG_BOARD_KEY:-ZENO_CORE}"
    local expected="${BOARD_PSRAM_MODE[$board_key]:-OCT}"          # OCT | QUAD
    local other="QUAD"; [[ "$expected" == "QUAD" ]] && other="OCT"
    local expected_label="${BOARD_PSRAM_LABEL[$board_key]:-Octal SPI (OPI)}"
    local board_label="${BOARD_LABEL[$board_key]:-Zeno Core}"

    if [[ ! -f "$SDKCONFIG_ZENO" ]]; then
        ui_msgbox "Board Configuration" \
            "sdkconfig.zeno not found:\n\n$SDKCONFIG_ZENO\n\n\
...
Cannot verify or repair without it."
    ...
    if board_config_check; then
        ...
        lines+=("")
        lines+=("  Board: $board_label")
        lines+=("  File:  $SDKCONFIG_ZENO")
        lines+=("")
        lines+=("  CONFIG_SPIRAM_MODE_${expected}=y                [OK]")
        lines+=("  CONFIG_SPIRAM_MODE_${other} not set          [OK]")
        lines+=("")
        lines+=("  Your $expected_label PSRAM configuration is valid.")
    ...
    local expected_status="MISSING" other_status="OK"
    grep -q "CONFIG_SPIRAM_MODE_${expected}=y" "$SDKCONFIG_ZENO" 2>/dev/null && \
        expected_status="OK"
    grep -q "CONFIG_SPIRAM_MODE_${other}=y" "$SDKCONFIG_ZENO" 2>/dev/null && \
        other_status="SET (WRONG)"

    local -a diag=()
    diag+=("")
    diag+=("  ⚠  Board Configuration Issue Detected")
    diag+=("")
    diag+=("  Board: $board_label")
    diag+=("  File:  $SDKCONFIG_ZENO")
    diag+=("")
    diag+=("  Diagnosis:")
    diag+=("  ──────────")
    diag+=("  CONFIG_SPIRAM_MODE_${expected}      → $expected_status")
    diag+=("  CONFIG_SPIRAM_MODE_${other}     → $other_status")
    diag+=("")
    diag+=("  Expected for $board_label ($expected_label):")
    diag+=("    CONFIG_SPIRAM_MODE_${expected}=y")
    diag+=("    CONFIG_SPIRAM_MODE_${other} must NOT be set")
    diag+=("")
    diag+=("  Impact:")
    diag+=("  Using incorrect PSRAM mode will cause a panic or")
    diag+=("  silent memory corruption at boot. $expected_label PSRAM")
    diag+=("  cannot be driven with ${other,,} mode settings.")
    diag+=("")
    diag+=("  Repair will:")
    diag+=("  1. Create a timestamped backup of the original file")
    diag+=("  2. Set CONFIG_SPIRAM_MODE_${expected}=y")
    diag+=("  3. Disable CONFIG_SPIRAM_MODE_${other}")
    diag+=("")

    ui_status_screen "Board Configuration — Issue Found" "${diag[@]}"

    if ui_yesno "Repair Board Configuration?" \
        "Configuration issue detected.\n\nRepair sdkconfig.zeno now?\n\nA timestamped backup will be created first."; then

        ui_wait_screen "Repairing Configuration" \
            "Writing corrected sdkconfig.zeno…"

        if board_config_repair; then
            ui_msgbox "Repair Complete" \
                "Configuration repaired successfully.\n\n\
Backup created at:\n$SDKCONFIG_ZENO.backup_*\n\n\
Repair log:\n$LOG_REPAIR"
        else
...
# =============================================================================
# §07b  BOARD DEFINITIONS  (Board Selection)
# =============================================================================
# ZENO_CORE and ZENO_FORGE are two real, separate MicroPython board
# directories (ports/esp32/boards/ZENO_CORE, ports/esp32/boards/ZENO_FORGE).
# Each board owns its own static sdkconfig.zeno with the correct flash mode/
# size/PSRAM options already baked in — Board Selection does not write or
# guess those values, it only:
#   1. Points BOARD/BUILD_DIR/BOARD_DIR/etc. at the chosen board's real
#      directory (_board_resolve_paths, §00), so `make BOARD=$CFG_BOARD_KEY`
#      and every derived path (firmware output, mpconfigboard.h, sdkconfig.zeno)
#      always agree with what was actually selected.
#   2. Sanity-checks that the board's own sdkconfig.zeno has the PSRAM mode
#      it's supposed to have (board_config_check, §07) and warns if not —
#      it never fabricates or overwrites a board's config to fake a profile.
#   3. Wires up the board's partition table via the existing Partition
#      Manager (_partition_apply, §C).
#
# To add a future board: add one key to each BOARD_* map below, and create
# ports/esp32/boards/<KEY>/ with the usual MicroPython board files. Nothing
# else in this file needs to change.
# =============================================================================

declare -A BOARD_LABEL=(
    [ZENO_CORE]="Zeno Core"
    [ZENO_FORGE]="Zeno Forge"
)
declare -A BOARD_DESC=(             # long-form descriptor, menu use only
    [ZENO_CORE]="ESP32-S3"
    [ZENO_FORGE]="ESP32-S3 + FPGA"
)
declare -A BOARD_FLASH_SIZE_LABEL=(
    [ZENO_CORE]="16 MiB"
    [ZENO_FORGE]="8 MiB"
)
declare -A BOARD_FLASH_SIZE_MB=(       # esptool --flash_size argument
    [ZENO_CORE]="16MB"
    [ZENO_FORGE]="8MB"
)
declare -A BOARD_FLASH_MODE=(          # esptool --flash_mode argument
    [ZENO_CORE]="qio"
    [ZENO_FORGE]="dio"
)
declare -A BOARD_PSRAM_LABEL=(
    [ZENO_CORE]="Octal SPI (OPI)"
    [ZENO_FORGE]="Quad SPI (QSPI)"
)
declare -A BOARD_PSRAM_MODE=(          # OCT | QUAD  → CONFIG_SPIRAM_MODE_*
    [ZENO_CORE]="OCT"
    [ZENO_FORGE]="QUAD"
)
declare -A BOARD_PARTITION_CSV=(
    [ZENO_CORE]="partitions-16MiB-zeno.csv"
    [ZENO_FORGE]="partitions-8MiB-zeno.csv"
)
declare -A BOARD_HAS_FPGA=(            # 1 = show "FPGA Support: Enabled"
    [ZENO_CORE]="0"
    [ZENO_FORGE]="1"
)

# ── board_key_valid KEY → 0/1 ────────────────────────────────────────────────
board_key_valid() {
    [[ -n "${BOARD_LABEL[$1]+x}" ]]
}

# ── board_build_summary_lines KEY → prints the Build Summary block ──────────
# Shared by board_selection_ui() and the pre-build confirmation screens.
board_build_summary_lines() {
    local key="$1"
    local -a s=()
    s+=("====================================================")
    s+=("Zeno OS Build Configuration")
    s+=("====================================================")
    s+=("Board            : ${BOARD_LABEL[$key]}")
    s+=("Flash            : ${BOARD_FLASH_SIZE_LABEL[$key]}")
    s+=("Flash Mode       : ${BOARD_FLASH_MODE[$key]^^}")
    s+=("PSRAM            : ${BOARD_PSRAM_LABEL[$key]}")
    s+=("Partition Table  : ${BOARD_PARTITION_CSV[$key]}")
    if [[ "${BOARD_HAS_FPGA[$key]}" == "1" ]]; then
        s+=("FPGA Support     : Enabled")
    fi
    s+=("====================================================")
    printf '%s\n' "${s[@]}"
}

# ── board_apply_selection KEY → 0/1 ──────────────────────────────────────────
# Switches the active board: repoints every board-dependent path at the real
# ports/esp32/boards/<KEY> directory, sanity-checks that board's own static
# sdkconfig.zeno, wires its partition table, and persists the choice.
board_apply_selection() {
    local key="$1"
    board_key_valid "$key" || { ui_msgbox "Board Selection" "Unknown board key: $key"; return 1; }

    CFG_BOARD_KEY="$key"
    _board_resolve_paths   # BOARD / BUILD_DIR / BOARD_DIR / SDKCONFIG_ZENO / FW_* now point at $key

    if [[ ! -d "$BOARD_DIR" ]]; then
        ui_msgbox "Board Selection" \
            "Board directory not found:\n$BOARD_DIR\n\n\
This board is not yet set up in the repository.\n\
The choice will still be recorded, but builds will fail until\n\
ports/esp32/boards/$key exists."
    elif [[ ! -f "$SDKCONFIG_ZENO" ]]; then
        ui_msgbox "Board Selection" \
            "sdkconfig.zeno not found for this board:\n$SDKCONFIG_ZENO\n\n\
Board choice recorded, but flash/PSRAM settings could not be verified."
    elif ! board_config_check; then
        ui_msgbox "Board Selection" \
            "Warning: $SDKCONFIG_ZENO does not match the expected PSRAM mode\n\
for ${BOARD_LABEL[$key]} (${BOARD_PSRAM_LABEL[$key]}).\n\n\
Board choice recorded. Use menu 7 — Repair Board Configuration —\n\
to fix sdkconfig.zeno before building."
    fi

    # Wire the partition table the same way the Partition Manager does.
    # (Requires the CSV to actually exist under ports/esp32/, same rule
    # already enforced by _partition_apply for every other board.)
    _partition_apply "${BOARD_PARTITION_CSV[$key]}"

    cfg_save   # ensure CFG_BOARD_KEY is persisted alongside CFG_PARTITION_CSV
...
board_selection_ui() {
    local -a items=(
        "ZENO_CORE"  "1. ${BOARD_LABEL[ZENO_CORE]} — ${BOARD_DESC[ZENO_CORE]}  (16 MiB / QIO / Octal PSRAM)"
        "ZENO_FORGE" "2. ${BOARD_LABEL[ZENO_FORGE]} — ${BOARD_DESC[ZENO_FORGE]}  (8 MiB / DIO / Quad PSRAM)"
        "BACK"       "← Return to main menu"
    )

    local choice
    choice=$(ui_menu "Board Selection  [current: ${BOARD_LABEL[$CFG_BOARD_KEY]:-unset}]" "${items[@]}")

    case "$choice" in
        ZENO_CORE|ZENO_FORGE)
            local summary; summary=$(board_build_summary_lines "$choice")
            if ui_yesno "Confirm Board Selection" \
                "$summary\n\nApply this configuration?\n\nA clean build is required afterwards."; then
                if board_apply_selection "$choice"; then
                    ui_msgbox "Board Selection Applied" \
                        "$summary\n\nUse: Build Automation → Clean Build to rebuild."
                fi
            fi
            ;;
        BACK|"") return 0 ;;
...
then re-launch the firmware manager."
    ...
    local _board_summary; _board_summary=$(board_build_summary_lines "${CFG_BOARD_KEY:-ZENO_CORE}")
    ...
    make BOARD="$CFG_BOARD_KEY" >> "$log_file" 2>&1 &
    local make_pid=$!
    ...
    (
        ...
        echo 100
    ) | ui_gauge "Building Firmware" "make BOARD=$CFG_BOARD_KEY — see $log_file"
    ...
    if [[ $rc -eq 0 ]]; then
        ...
        summary+=("")
        summary+=("  Board      : ${BOARD_LABEL[$CFG_BOARD_KEY]:-$CFG_BOARD_KEY}")
...
_flash_summary_screen() {
    ...
    local _flash_mode="${BOARD_FLASH_MODE[$CFG_BOARD_KEY]:-dio}"
    local _flash_size="${BOARD_FLASH_SIZE_MB[$CFG_BOARD_KEY]:-16MB}"
    ...
    lines+=("  ────────────────────────────────────────────────")
    lines+=("")
    lines+=("  Target Board  : ${BOARD_LABEL[$CFG_BOARD_KEY]:-$CFG_BOARD_KEY}")
    ...
    lines+=("  ────────────────")
    lines+=("  Chip          : esp32s3")
    lines+=("  Flash mode    : $_flash_mode")
    lines+=("  Flash size    : $_flash_size")
...
flash_firmware() {
...

    {
        ...
        echo "# Port     : $port"
        echo "# Baud     : $CFG_BAUD"
        echo "# Board    : $CFG_BOARD_KEY (${BOARD_LABEL[$CFG_BOARD_KEY]:-unset})"
        echo "# ────────────────────────────────────────────"
        echo ""
    } > "$log_file"

    # ── Flash mode / size come from the selected board (§07b) ─────────────────
    # Falls back to the original dio/16MB defaults if no board is selected yet
    # (e.g. settings file predates Board Selection).
    local _flash_mode="${BOARD_FLASH_MODE[$CFG_BOARD_KEY]:-dio}"
    local _flash_size="${BOARD_FLASH_SIZE_MB[$CFG_BOARD_KEY]:-16MB}"
    ...
    if [[ $rc -eq 0 ]]; then
        ui_msgbox "Flash Complete" \
            "✔  Firmware flashed successfully!\n\nPort  : $port\nBoard : ${BOARD_LABEL[$CFG_BOARD_KEY]:-$CFG_BOARD_KEY}\nLog   : $log_file"
...
build_info_ui() {
    ...
    # PSRAM config summary — correctness is relative to the SELECTED board,
    # not a fixed assumption (Octal for Core, Quad for Forge are both valid).
    local _expected_psram="${BOARD_PSRAM_MODE[$CFG_BOARD_KEY]:-OCT}"
    local psram_mode="unknown"
    if [[ -f "$SDKCONFIG_ZENO" ]]; then
        if grep -q 'CONFIG_SPIRAM_MODE_OCT=y' "$SDKCONFIG_ZENO"; then
            if [[ "$_expected_psram" == "OCT" ]]; then
                psram_mode="Octal (OPI) ✔"
            else
                psram_mode="Octal (WRONG — ${BOARD_LABEL[$CFG_BOARD_KEY]} expects ${BOARD_PSRAM_LABEL[$CFG_BOARD_KEY]})"
            fi
        elif grep -q 'CONFIG_SPIRAM_MODE_QUAD=y' "$SDKCONFIG_ZENO"; then
            if [[ "$_expected_psram" == "QUAD" ]]; then
                psram_mode="Quad (QSPI) ✔"
            else
                psram_mode="Quad (WRONG — ${BOARD_LABEL[$CFG_BOARD_KEY]} expects ${BOARD_PSRAM_LABEL[$CFG_BOARD_KEY]})"
            fi
        else
            psram_mode="unknown (check $SDKCONFIG_ZENO)"
        fi
    else
        psram_mode="unknown (sdkconfig.zeno not found)"
    fi

    # Flash config — from the selected board, not a fixed literal
    local flash_mode="${BOARD_FLASH_MODE[$CFG_BOARD_KEY]:-unknown} / 80MHz / ${BOARD_FLASH_SIZE_LABEL[$CFG_BOARD_KEY]:-unknown}"
    ...
    lines+=("  Build")
    lines+=("  ─────")
    lines+=("  Board           : ${BOARD_LABEL[$CFG_BOARD_KEY]:-$CFG_BOARD_KEY} ($CFG_BOARD_KEY)")
    ...
    lines+=("  ──────────────────────")
    lines+=("  Target hardware : ${BOARD_DESC[$CFG_BOARD_KEY]:-ESP32-S3} (${BOARD_LABEL[$CFG_BOARD_KEY]:-$CFG_BOARD_KEY})")
...
settings_ui() {
    while true; do
        local -a items=(
            "BOARD"    "Board                [${BOARD_LABEL[$CFG_BOARD_KEY]:-unset}]  → opens Board Selection"
        ...
        case "$choice" in
            BOARD)
                # Board choice is validated against BOARD_LABEL — free-text entry
                # here would let a typo silently break BUILD_DIR/BOARD_DIR/etc,
                # so this always goes through the same picker as main menu → 12.
                board_selection_ui
                ;;
            PORT)
...
build_clean() {
    ...
    if ! ui_yesno "Clean Build" \
        "$_board_summary\n\n\
This will:\n\
  1. Delete build-$CFG_BOARD_KEY/ completely\n\
  2. Run: make BOARD=$CFG_BOARD_KEY submodules\n\
  3. Run: make BOARD=$CFG_BOARD_KEY\n\n\
Estimated time: 3–10 minutes."; then
    ...
    # Step 2: submodules
    log_write "$log_file" CMD "make BOARD=$CFG_BOARD_KEY submodules"
    {
        echo "# ── make submodules ──"
        make BOARD="$CFG_BOARD_KEY" submodules 2>&1
        echo "# ── submodules done ──"
    } >> "$log_file"

    # Step 3: main build with progress gauge
    log_write "$log_file" CMD "make BOARD=$CFG_BOARD_KEY"
    ...
    (
        ...
        echo 100
    ) | ui_gauge "Clean Build in Progress" "Building for $CFG_BOARD_KEY"
...
Use this for day-to-day development."; then
    ...
    (
        ...
        echo 100
    ) | ui_gauge "Building (Incremental)" "make BOARD=$CFG_BOARD_KEY"
...
_build_show_result() {
    ...
    if [[ "$rc" -eq 0 ]]; then
        ...
        ok+=("  Board      : ${BOARD_LABEL[$CFG_BOARD_KEY]:-$CFG_BOARD_KEY}")
...
# =============================================================================
# §15b  LOGIN / AUTHENTICATION
# =============================================================================

readonly ZFM_LOGIN_PASSWORD="Shanmukha2007"
readonly ZFM_LOGIN_MAX_ATTEMPTS=3

# ── login_gate — prompt for password before allowing access ────────────────
# Exits the script if authentication fails after ZFM_LOGIN_MAX_ATTEMPTS tries.
login_gate() {
    local attempt=1 entered
    while (( attempt <= ZFM_LOGIN_MAX_ATTEMPTS )); do
        entered=$(ui_passwordbox "ZENO OS Firmware Manager — Login" \
            "Enter password to continue (attempt $attempt of $ZFM_LOGIN_MAX_ATTEMPTS):")

        if [[ "$entered" == "$ZFM_LOGIN_PASSWORD" ]]; then
            return 0
        fi

        (( attempt++ ))
        if (( attempt <= ZFM_LOGIN_MAX_ATTEMPTS )); then
            ui_msgbox "Login Failed" "Incorrect password. Please try again."
        fi
    done

    ui_msgbox "Access Denied" "Too many failed attempts. Exiting."
    exit 1
}
...
show_welcome() {
    ...
    msg+="  UI        : $UI_BACKEND\n"
    msg+="\n"
    msg+="  Board     : $CFG_BOARD_KEY (${BOARD_LABEL[$CFG_BOARD_KEY]:-unset})\n"
    msg+="  Target    : ${BOARD_FLASH_SIZE_LABEL[$CFG_BOARD_KEY]:-16 MiB} / ${BOARD_FLASH_MODE[$CFG_BOARD_KEY]:-qio} / ${BOARD_PSRAM_LABEL[$CFG_BOARD_KEY]:-Octal SPI (OPI)}\n"
    msg+="\n"
    msg+="  Capabilities:\n"
...
main_menu() {
    while true; do
        ...
        local -a items=(
            ...
            "12" "Board Selection             [${BOARD_LABEL[$CFG_BOARD_KEY]:-unset}]"
            "13" "Settings"
            "14" "Exit"
        )
