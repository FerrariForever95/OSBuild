...
# ── GitHub Integration ───────────────────────────────────────────────────────
readonly GITHUB_REPO_NAME="Zeno-OS"
readonly GITHUB_BRANCH="main"

# ── ESP32 Filesystem Manager ─────────────────────────────────────────────────
readonly FS_LOCAL_DEFAULT="$REPO/device-fs"          # local mirror of device filesystem
readonly FS_MANIFEST_FILE="$REPO/.config/fs_manifest.conf"   # last-synced hash manifest

# ── SDKCONFIG Manager ────────────────────────────────────────────────────────
readonly SDKCONFIG_DEFAULTS_FILE="$PORT_DIR/sdkconfig.defaults" # baseline for "restore defaults"
SDKCFG_ACTIVE_FILE=""    # currently open sdkconfig file (set by sdkcfg_manager_ui)
...
# ── _fmt_bytes BYTES  →  human readable size (e.g. 4.2MB) ───────────────────
_fmt_bytes() {
    local b="${1:-0}"
    awk -v b="$b" 'BEGIN {
        split("B KB MB GB", u, " ")
...
# ── _fmt_eta SECONDS  →  human readable duration (e.g. 1m32s) ───────────────
_fmt_eta() {
    local s="${1:-0}"
    [[ "$s" =~ ^[0-9]+$ ]] || s=0
    local m=$(( s / 60 )) rem=$(( s % 60 ))
    if [[ $m -gt 0 ]]; then
...
# ── _build_progress_pipeline LOG_FILE TITLE CMD...  →  returns CMD exit code
# Runs CMD in the background, appending its output to LOG_FILE, and drives
# a progress gauge from REAL build progress.  ESP-IDF's ninja backend (which
# `make` shells out to for this port) prints lines of the form:
#     [123/2481] Building C object esp-idf/.../foo.c.obj
# We tail the growing log file and parse the most recent "[cur/total]" marker
# to compute an accurate percentage. If no such marker has appeared yet
# (e.g. during CMake configure or dependency-download phases), we fall back
# to a conservative time-based asymptotic estimate that never exceeds 95%,
# so the bar never looks "finished" before the tool actually finishes.
_build_progress_pipeline() {
    local log_file="$1"; shift
    local title="$1"; shift
    ...
    (
        ...
        while kill -0 "$cmd_pid" 2>/dev/null; do
            ...
            line=$(grep -oE '\[[[:space:]]*[0-9]+/[0-9]+\]' "$log_file" 2>/dev/null | tail -1)
            if [[ -n "$line" ]]; then
                cur=$(echo "$line" | grep -oE '[0-9]+' | head -1)
                tot=$(echo "$line" | grep -oE '[0-9]+' | tail -1)
                if [[ -n "$cur" && -n "$tot" && "$tot" -gt 0 ]]; then
                    local real_pct=$(( cur * 100 / tot ))
                    (( real_pct > 99 )) && real_pct=99
                    (( real_pct < last_pct )) && real_pct=$last_pct
                    last_pct=$real_pct
                    echo "$last_pct"
                    continue
                fi
            ...
            # Fallback: asymptotic time-based estimate, capped below 95%
            local pct
            pct=$(echo "scale=0; 95 - (95 / (1 + $elapsed / 30))" | bc 2>/dev/null || \
                  echo $(( 95 < elapsed*2 ? 95 : elapsed*2 )) )
            [[ $pct -lt $last_pct ]] && pct=$last_pct
            last_pct=$pct
            echo "$pct"
        done
        echo 100
    ) | ui_gauge "$title" "See $log_file"
...
build_firmware() {
    ...
    local suppress_git="${2:-0}"  # 1 = skip the post-build GitHub offer
...
then re-launch the firmware manager."
    ...
    # Real progress: parsed from ninja "[cur/total]" markers as they land
    # in the log file, with a safe time-based fallback for phases (CMake
    # configure, dependency checks) that don't emit that marker.
    _build_progress_pipeline "$log_file" "Building Firmware" \
        make BOARD="$CFG_BOARD"
    rc=$?
    ...
    if [[ $rc -eq 0 ]]; then
        ...
        [[ "$suppress_git" != "1" ]] && git_offer_commit "Build"
        return 0
    else
...
# ── _flash_progress_pipeline LOG_FILE TITLE CMD...  →  returns CMD exit code
# esptool prints lines such as:
#     Writing at 0x00010000... (12 %)
# We tail the log file for the most recent "(NN %)" marker and drive the
# gauge from that value directly — this is esptool's own self-reported
# percentage of the write_flash operation, not a time-based guess.
_flash_progress_pipeline() {
    local log_file="$1"; shift
    local title="$1"; shift
    ...
    (
        ...
        while kill -0 "$cmd_pid" 2>/dev/null; do
            sleep 1
            pct=$(grep -oE '\([0-9]+ ?%\)' "$log_file" 2>/dev/null | tail -1 | grep -oE '[0-9]+')
            if [[ -n "$pct" ]]; then
...
flash_firmware() {
    ...
    # Show summary and ask for confirmation (skipped when running silently,
    # e.g. from Full Automated Workflow or Quick Build)
    if [[ "$silent" == "0" ]]; then
    ...
    local rc=0
    _flash_progress_pipeline "$log_file" "Flashing Firmware" "${cmd[@]}"
    rc=$?
...
Estimated time: 3–10 minutes."; then
    ...
    # Step 3: main build with real progress gauge
    log_write "$log_file" CMD "make BOARD=$CFG_BOARD"

    local build_start; build_start=$(date +%s)
    local rc=0

    _build_progress_pipeline "$log_file" "Clean Build in Progress" \
        make BOARD="$CFG_BOARD"
    rc=$?
...
Use this for day-to-day development."; then
    ...
    _build_progress_pipeline "$log_file" "Building (Incremental)" \
        make BOARD="$CFG_BOARD"
    rc=$?
...
# =============================================================================
# §E  ESP32 FILESYSTEM MANAGER
# =============================================================================
# Uses mpremote to move files between a local mirror directory and the
# device's on-board filesystem, preserving directory structure.  All
# transfers show real progress: current file, bytes transferred, transfer
# speed, and ETA — driven by ui_gauge_fs, which understands whiptail/dialog's
# native "XXX / pct / message / XXX" gauge update protocol.
# =============================================================================

FS_LOCAL_DIR="$FS_LOCAL_DEFAULT"   # adjustable via the Filesystem Manager menu

# ── ui_gauge_fs TITLE  ────────────────────────────────────────────────────────
# Like ui_gauge, but reads the richer whiptail/dialog gauge update protocol:
#   XXX
#   <percent>
#   <message line 1>
#   <message line 2>
#   ...
#   XXX
# whiptail and dialog already understand this natively when piped to
# --gauge, so those backends are simple passthroughs. The text backend
# parses the block itself and re-renders it as a single overwritten line.
ui_gauge_fs() {
    local title="$1"
    case "$UI_BACKEND" in
        whiptail) whiptail --title " $title " --gauge "Preparing…" 9 "$UI_W" 0 ;;
        dialog)   dialog   --title " $title " --gauge "Preparing…" 9 "$UI_W" 0 ;;
        text)
            local line pct="" msgline="" in_block=0
            while IFS= read -r line; do
                if [[ "$line" == "XXX" ]]; then
                    if [[ $in_block -eq 0 ]]; then
                        in_block=1; pct=""; msgline=""
                    else
                        in_block=0
                        printf "\r  %-90s [%3s%%]" "${msgline:0:90}" "${pct:-0}"
                    fi
                    continue
                fi
...
# ── _fs_ensure_port  →  sets SELECTED_PORT or returns 1 ─────────────────────
_fs_ensure_port() {
...
# ── _fs_ensure_mpremote  →  0 if usable, else shows error and returns 1 ─────
_fs_ensure_mpremote() {
    if python3 -m mpremote version >/dev/null 2>&1; then
        return 0
    fi
    ui_msgbox "mpremote Not Found" \
        "mpremote is required for filesystem operations.\n\nInstall with:\n  pip install mpremote"
    return 1
}

# ── _fs_mkdir_remote_recursive PORT REMOTE_DIR  ──────────────────────────────
# mpremote's `fs mkdir` is not recursive, so we build up each path segment.
# Failures (directory already exists) are silently ignored.
_fs_mkdir_remote_recursive() {
    local port="$1" remote_dir="$2"
    local path="${remote_dir#:}"
    path="${path#/}"
    ...
    for part in "${parts[@]}"; do
        [[ -z "$part" ]] && continue
        accum="$accum/$part"
        python3 -m mpremote connect "$port" fs mkdir ":$accum" >/dev/null 2>&1 || true
    done
}

# ── _fs_remote_walk PORT  →  prints "TYPE\tPATH\tSIZE" lines for every entry
# TYPE is F (file) or D (directory).  Uses os.ilistdir() via mpremote exec,
# which works across MicroPython versions without depending on any
# mpremote "find" subcommand that may or may not exist.
_fs_remote_walk() {
    local port="$1"
    local script
    script=$(cat <<'PYEOF'
    ...
    for entry in entries:
        ...
        if etype == 0x4000:
            ...
            try:
                ...
                size = 0
            print("F\t" + full + "\t" + str(size))
_walk("/")
PYEOF
)
    python3 -m mpremote connect "$port" exec "$script" 2>/dev/null
}

# ── _fs_transfer_loop DIRECTION PORT LOG_FILE FILE_LIST[]  ───────────────────
# DIRECTION = "upload" | "download"
# FILE_LIST elements are "relpath\tsize" pairs (relative to FS_LOCAL_DIR).
# Drives ui_gauge_fs with real byte-based progress, current file, transfer
# speed and ETA.  Returns 0 if every file transferred successfully.
_fs_transfer_loop() {
    local direction="$1" port="$2" log_file="$3"; shift 3
    local -a entries=("$@")

    local total_bytes=0 f rel sz
    for f in "${entries[@]}"; do
        sz="${f##*$'\t'}"
        [[ "$sz" =~ ^[0-9]+$ ]] && total_bytes=$(( total_bytes + sz ))
    done
    [[ $total_bytes -eq 0 ]] && total_bytes=1   # avoid div-by-zero

    local total_files=${#entries[@]}
    local fail_count=0

    (
        local bytes_done=0 idx=0
        ...
        for f in "${entries[@]}"; do
            ...
            if [[ "$direction" == "upload" ]]; then
                local local_path="$FS_LOCAL_DIR/$rel"
                local remote_path=":$rel"
                local remote_dir
                remote_dir=$(dirname "$remote_path")
                _fs_mkdir_remote_recursive "$port" "$remote_dir"
                if ! python3 -m mpremote connect "$port" fs cp "$local_path" "$remote_path" \
                        >> "$log_file" 2>&1; then
                    status="FAIL"
                fi
            else
                ...
                if ! python3 -m mpremote connect "$port" fs cp "$remote_path" "$local_path" \
                        >> "$log_file" 2>&1; then
                    status="FAIL"
                fi
            fi

            echo "[$(date '+%H:%M:%S')] [$status] $direction  $rel  (${sz}B)" >> "$log_file"

            bytes_done=$(( bytes_done + sz ))
            local elapsed=$(( $(date +%s) - start_ts ))
            [[ $elapsed -lt 1 ]] && elapsed=1
            local speed_bps=$(( bytes_done / elapsed ))
            local remain_bytes=$(( total_bytes - bytes_done ))
            (( remain_bytes < 0 )) && remain_bytes=0
            local eta_s=0
            [[ $speed_bps -gt 0 ]] && eta_s=$(( remain_bytes / speed_bps ))
            local pct=$(( bytes_done * 100 / total_bytes ))
            (( pct > 99 )) && pct=99

            printf 'XXX\n%d\n[%d/%d] %s %s (%s)\nSpeed: %s/s   ETA: %s   Transferred: %s / %s\nXXX\n' \
                "$pct" "$idx" "$total_files" "$status" "$rel" "$(_fmt_bytes "$sz")" \
                "$(_fmt_bytes "$speed_bps")" "$(_fmt_eta "$eta_s")" \
                "$(_fmt_bytes "$bytes_done")" "$(_fmt_bytes "$total_bytes")"
        done
        printf 'XXX\n100\nTransfer complete — %d file(s)\n\nXXX\n' "$total_files"
    ) | ui_gauge_fs "$([[ "$direction" == "upload" ]] && echo "Uploading Filesystem" || echo "Downloading Filesystem") — $port"
...
# ── fs_upload_all  ────────────────────────────────────────────────────────────
fs_upload_all() {
    _fs_ensure_mpremote || return 1
    _fs_ensure_port || return 1
    local port="$SELECTED_PORT"

    if [[ ! -d "$FS_LOCAL_DIR" ]]; then
        ui_msgbox "Local Directory Missing" \
            "Local filesystem directory not found:\n$FS_LOCAL_DIR\n\nCreate it and populate it first, or change it in Filesystem Manager → Set Local Directory."
        return 1
    fi

    local -a files
    mapfile -t files < <(cd "$FS_LOCAL_DIR" && find . -type f | sed 's|^\./||' | sort)
    if [[ ${#files[@]} -eq 0 ]]; then
        ui_msgbox "Nothing to Upload" "No files found under:\n$FS_LOCAL_DIR"
        return 1
    fi

    if ! ui_yesno "Upload Entire Filesystem" \
        "Upload ${#files[@]} file(s) from:\n  $FS_LOCAL_DIR\nto device on $port?\n\nExisting files with the same name will be overwritten."; then
        return 0
    fi
    ...
    local rel sz
    for rel in "${files[@]}"; do
        sz=$(stat -c '%s' "$FS_LOCAL_DIR/$rel" 2>/dev/null || echo 0)
        entries+=("$rel"$'\t'"$sz")
    done

    local ts; ts=$(date '+%Y%m%d_%H%M%S')
    LOG_FS="$LOG_DIR/fs_${ts}.log"
    mkdir -p "$LOG_DIR"
    {
        echo "# ZENO OS Firmware Manager — Filesystem Upload Log"
        echo "# Started : $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
        echo "# Port    : $port"
        echo "# Local   : $FS_LOCAL_DIR"
        echo "# Files   : ${#entries[@]}"
        echo "# ─────────────────────────────────────────"
    } > "$LOG_FS"

    if _fs_transfer_loop "upload" "$port" "$LOG_FS" "${entries[@]}"; then
        ui_msgbox "Upload Complete" \
            "✔  Uploaded ${#entries[@]} file(s) to $port.\n\nLog: $LOG_FS"
        _fs_update_manifest "${files[@]}"
        git_offer_commit "Filesystem Upload"
        return 0
    else
        ui_msgbox "Upload Finished With Errors" \
...
fs_download_all() {
    ...
    ui_infobox "Filesystem Manager" "Listing files on device…"
    local -a raw
    mapfile -t raw < <(_fs_remote_walk "$port")

    local -a entries=()
    local line type path size
    for line in "${raw[@]}"; do
        type="${line%%$'\t'*}"
        [[ "$type" != "F" ]] && continue
        local rest="${line#*$'\t'}"
    ...
    if [[ ${#entries[@]} -eq 0 ]]; then
        ui_msgbox "Nothing to Download" "No files found on device $port."
        return 1
    fi

    mkdir -p "$FS_LOCAL_DIR"

    if ! ui_yesno "Download Entire Filesystem" \
        "Download ${#entries[@]} file(s) from device $port to:\n  $FS_LOCAL_DIR\n\nExisting local files with the same name will be overwritten."; then
        return 0
    fi
    ...
    mkdir -p "$LOG_DIR"
    {
        echo "# ZENO OS Firmware Manager — Filesystem Download Log"
        echo "# Started : $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
        echo "# Port    : $port"
        echo "# Local   : $FS_LOCAL_DIR"
        echo "# Files   : ${#entries[@]}"
    ...
    if _fs_transfer_loop "download" "$port" "$LOG_FS" "${entries[@]}"; then
        ui_msgbox "Download Complete" \
            "✔  Downloaded ${#entries[@]} file(s) from $port.\n\nLog: $LOG_FS"
        return 0
    else
        ui_msgbox "Download Finished With Errors" \
...
fs_upload_selected() {
    ...
    if [[ ! -d "$FS_LOCAL_DIR" ]]; then
        ui_msgbox "Local Directory Missing" "Not found:\n$FS_LOCAL_DIR"
        return 1
    fi
    ...
    local rel
    for rel in "${files[@]}"; do
        checklist_items+=("$rel" "$rel" "off")
    done

    local selection
    selection=$(ui_checklist "Select Files to Upload" "${checklist_items[@]}")
    [[ -z "$selection" ]] && return 0

    local -a chosen=()
    eval "local -a _sel=($selection)"
    for rel in "${_sel[@]}"; do
        chosen+=("$rel")
    done

    if [[ ${#chosen[@]} -eq 0 ]]; then
        ui_msgbox "Nothing Selected" "No files were selected."
        return 0
    fi
    ...
    mkdir -p "$LOG_DIR"
    {
        echo "# ZENO OS Firmware Manager — Selected Upload Log"
        echo "# Started : $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
        echo "# Port    : $port"
        echo "# Files   : ${#entries[@]}"
        echo "# ─────────────────────────────────────────"
    ...
    if _fs_transfer_loop "upload" "$port" "$LOG_FS" "${entries[@]}"; then
        ui_msgbox "Upload Complete" "✔  Uploaded ${#entries[@]} file(s).\n\nLog: $LOG_FS"
        _fs_update_manifest "${chosen[@]}"
        git_offer_commit "Filesystem Upload"
    else
        ui_msgbox "Upload Finished With Errors" "Check the log:\n$LOG_FS"
    fi
}

# ── fs_download_selected  ─────────────────────────────────────────────────────
fs_download_selected() {
    _fs_ensure_mpremote || return 1
    _fs_ensure_port || return 1
    local port="$SELECTED_PORT"
    ...
    local selection
    selection=$(ui_checklist "Select Files to Download" "${checklist_items[@]}")
    [[ -z "$selection" ]] && return 0
    ...
    mkdir -p "$LOG_DIR"
    {
        echo "# ZENO OS Firmware Manager — Selected Download Log"
        echo "# Started : $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
        echo "# Port    : $port"
        echo "# Files   : ${#entries[@]}"
        echo "# ─────────────────────────────────────────"
    ...
    if _fs_transfer_loop "download" "$port" "$LOG_FS" "${entries[@]}"; then
        ui_msgbox "Download Complete" "✔  Downloaded ${#entries[@]} file(s).\n\nLog: $LOG_FS"
    else
        ui_msgbox "Download Finished With Errors" "Check the log:\n$LOG_FS"
    fi
}

# ── Manifest helpers (used by Sync — Only Changed Files) ─────────────────────
# Manifest format: "relpath\tmd5hash" one per line.
_fs_manifest_load() {
    [[ -f "$FS_MANIFEST_FILE" ]] || return 0
    cat "$FS_MANIFEST_FILE"
}

_fs_manifest_get() {
    local rel="$1"
    [[ -f "$FS_MANIFEST_FILE" ]] || return 1
    awk -F'\t' -v r="$rel" '$1==r {print $2; found=1} END{exit !found}' "$FS_MANIFEST_FILE"
}

_fs_update_manifest() {
    local -a rels=("$@")
    mkdir -p "$(dirname "$FS_MANIFEST_FILE")"
    local tmp; tmp=$(mktemp /tmp/zfm_manifest_XXXX)
    [[ -f "$FS_MANIFEST_FILE" ]] && cp "$FS_MANIFEST_FILE" "$tmp" || : > "$tmp"

    local rel hash
    for rel in "${rels[@]}"; do
        [[ -f "$FS_LOCAL_DIR/$rel" ]] || continue
        hash=$(md5sum "$FS_LOCAL_DIR/$rel" 2>/dev/null | awk '{print $1}')
        [[ -z "$hash" ]] && continue
        if grep -q $'^'"$rel"$'\t' "$tmp" 2>/dev/null; then
            sed -i "s|^${rel}\t.*|${rel}\t${hash}|" "$tmp"
        else
            printf '%s\t%s\n' "$rel" "$hash" >> "$tmp"
        fi
    done
    mv "$tmp" "$FS_MANIFEST_FILE"
}

# ── fs_sync_changed  ───────────────────────────────────────────────────────────
# Uploads only files whose content hash differs from the last recorded
# manifest entry (or that have never been synced before).
fs_sync_changed() {
    _fs_ensure_mpremote || return 1
    _fs_ensure_port || return 1
    local port="$SELECTED_PORT"
    ...
    mapfile -t files < <(cd "$FS_LOCAL_DIR" && find . -type f | sed 's|^\./||' | sort)
    if [[ ${#files[@]} -eq 0 ]]; then
        ui_msgbox "Nothing to Sync" "No files found under:\n$FS_LOCAL_DIR"
        return 1
    fi

    ui_infobox "Sync" "Computing checksums for ${#files[@]} local file(s)…"

    local -a changed=()
    local rel hash old_hash
    for rel in "${files[@]}"; do
        hash=$(md5sum "$FS_LOCAL_DIR/$rel" 2>/dev/null | awk '{print $1}')
        old_hash=$(_fs_manifest_get "$rel" 2>/dev/null || echo "")
    ...
    if [[ ${#changed[@]} -eq 0 ]]; then
        ui_msgbox "Sync — Up To Date" \
            "All ${#files[@]} local file(s) already match the last-synced manifest.\n\nNothing to upload."
        return 0
    fi

    if ! ui_yesno "Sync Changed Files" \
        "${#changed[@]} of ${#files[@]} file(s) have changed since the last sync.\n\nUpload just the changed files to $port?"; then
        return 0
    fi
    ...
    mkdir -p "$LOG_DIR"
    {
        echo "# ZENO OS Firmware Manager — Sync (Changed Files Only) Log"
        echo "# Started : $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
        echo "# Port    : $port"
        echo "# Changed : ${#entries[@]} of ${#files[@]}"
        echo "# ─────────────────────────────────────────"
    } > "$LOG_FS"

    if _fs_transfer_loop "upload" "$port" "$LOG_FS" "${entries[@]}"; then
        ui_msgbox "Sync Complete" \
            "✔  Synced ${#entries[@]} changed file(s) to $port.\n\nLog: $LOG_FS"
        _fs_update_manifest "${changed[@]}"
        git_offer_commit "Filesystem Sync"
    else
        ui_msgbox "Sync Finished With Errors" "Check the log:\n$LOG_FS"
    fi
}

# ── Filesystem Manager status screen ─────────────────────────────────────────
_fs_status_ui() {
    local -a lines=()
    lines+=("")
    lines+=("  Filesystem Manager — Status")
    lines+=("  ══════════════════════════════════════════")
    lines+=("")
    lines+=("  Local directory : $FS_LOCAL_DIR")
    if [[ -d "$FS_LOCAL_DIR" ]]; then
        local count; count=$(find "$FS_LOCAL_DIR" -type f 2>/dev/null | wc -l)
        local size;  size=$(du -sh "$FS_LOCAL_DIR" 2>/dev/null | cut -f1)
        lines+=("                    [EXISTS — $count file(s), $size]")
    else
        lines+=("                    [NOT FOUND]")
    fi
    lines+=("")
    lines+=("  Selected port   : ${SELECTED_PORT:-none — will auto-detect}")
    lines+=("")
    lines+=("  Manifest file   : $FS_MANIFEST_FILE")
    if [[ -f "$FS_MANIFEST_FILE" ]]; then
        local mcount; mcount=$(wc -l < "$FS_MANIFEST_FILE" 2>/dev/null || echo 0)
        lines+=("                    [EXISTS — $mcount tracked file(s)]")
    else
        lines+=("                    [none yet — first sync creates it]")
    fi
    lines+=("")
    lines+=("  mpremote        : $(python3 -m mpremote version 2>&1 | head -1)")
    lines+=("")
    ui_status_screen "Filesystem Manager Status" "${lines[@]}"
}
...
fs_manager_ui() {
    while true; do
        local -a items=(
            "UPLOAD_ALL"     "Upload entire filesystem  → device"
            "DOWNLOAD_ALL"   "Download entire filesystem ← device"
            "UPLOAD_SEL"     "Upload selected files/folders"
            "DOWNLOAD_SEL"   "Download selected files/folders"
            "SYNC"           "Sync only changed files → device"
            "STATUS"         "Status  [local: $FS_LOCAL_DIR]"
            "SETDIR"         "Set local filesystem directory"
            "BACK"           "← Return to main menu"
        )

        local choice
        choice=$(ui_menu "ESP32 Filesystem Manager" "${items[@]}")

        case "$choice" in
            UPLOAD_ALL)    fs_upload_all ;;
            DOWNLOAD_ALL)  fs_download_all ;;
            UPLOAD_SEL)    fs_upload_selected ;;
            DOWNLOAD_SEL)  fs_download_selected ;;
            SYNC)          fs_sync_changed ;;
            STATUS)        _fs_status_ui ;;
            SETDIR)
                local newdir
                newdir=$(ui_inputbox "Local Filesystem Directory" \
                    "Local directory to mirror the device filesystem:" "$FS_LOCAL_DIR")
                if [[ -n "$newdir" ]]; then
                    FS_LOCAL_DIR="$newdir"
                    mkdir -p "$FS_LOCAL_DIR" 2>/dev/null || true
                fi
                ;;
            BACK|"") return 0 ;;
        esac
...
# =============================================================================
# Offered automatically after a successful build or filesystem upload, and
# also available as a standalone menu entry. Repository: Zeno-OS, branch:
# main (both configurable via GITHUB_REPO_NAME / GITHUB_BRANCH constants).
# =============================================================================

# ── git_offer_commit CONTEXT_LABEL  ──────────────────────────────────────────
git_offer_commit() {
    ...
    if [[ -z "$status_out" ]]; then
        return 0    # nothing changed — don't bother the user
    fi

    if ! ui_yesno "GitHub Integration" \
        "$context completed successfully.\n\nThe repository has uncommitted changes.\n\nCommit and push to $GITHUB_REPO_NAME ($GITHUB_BRANCH) now?"; then
        return 0
    fi

    local -a mod_lines=()
    mod_lines+=("")
    mod_lines+=("  Modified Files")
    mod_lines+=("  ══════════════════════════════════════")
    mod_lines+=("")
    while IFS= read -r line; do
        mod_lines+=("  $line")
    ...
    mod_lines+=("")
    ui_status_screen "Git — Modified Files" "${mod_lines[@]}"

    if ! ui_yesno "Stage Changes" "Stage ALL changes with 'git add .'?"; then
        return 0
    fi
    ...
    mkdir -p "$LOG_DIR"
    {
        echo "# ZENO OS Firmware Manager — Git Integration Log"
        echo "# Started    : $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
    ...
    log_write "$LOG_GIT" CMD "git -C $REPO add ."
    if ! git -C "$REPO" add . >> "$LOG_GIT" 2>&1; then
        ui_msgbox "Git Add Failed" "git add . failed.\n\nCheck log: $LOG_GIT"
        return 1
    fi

    local default_msg
    default_msg="${context}: $(date '+%Y-%m-%d %H:%M:%S')"
    local commit_msg
    commit_msg=$(ui_inputbox "Commit Message" \
        "Edit the commit message if needed:" "$default_msg")
    [[ -z "$commit_msg" ]] && commit_msg="$default_msg"

    log_write "$LOG_GIT" CMD "git -C $REPO commit -m \"$commit_msg\""
    if ! git -C "$REPO" commit -m "$commit_msg" >> "$LOG_GIT" 2>&1; then
        ui_msgbox "Git Commit Failed" \
            "git commit failed (possibly nothing staged).\n\nCheck log: $LOG_GIT"
        return 1
    fi

    if ! ui_yesno "Push to GitHub?" \
        "Push the new commit to origin/$GITHUB_BRANCH now?\n\nRepository: $GITHUB_REPO_NAME"; then
        ui_msgbox "Commit Saved Locally" \
            "The commit was created but NOT pushed.\n\nLog: $LOG_GIT"
        return 0
    fi

    log_write "$LOG_GIT" CMD "git -C $REPO push origin $GITHUB_BRANCH"
    if git -C "$REPO" push origin "$GITHUB_BRANCH" >> "$LOG_GIT" 2>&1; then
        ui_msgbox "Push Complete" \
            "✔  Changes pushed to $GITHUB_REPO_NAME ($GITHUB_BRANCH).\n\nLog: $LOG_GIT"
        return 0
    else
        local -a perr=()
        perr+=("") perr+=("  ✘  git push FAILED") perr+=("")
        perr+=("  Repository : $GITHUB_REPO_NAME")
        perr+=("  Branch     : $GITHUB_BRANCH")
        perr+=("  Log        : $LOG_GIT")
        perr+=("")
        perr+=("  Last output:")
        perr+=("  ────────────")
        while IFS= read -r line; do perr+=("  $line"); done < <(tail -15 "$LOG_GIT")
        perr+=("")
        ui_status_screen "Git Push Failed" "${perr[@]}"
        return 1
    fi
}

# ── github_integration_ui  — standalone menu entry ───────────────────────────
github_integration_ui() {
    command -v git >/dev/null 2>&1 || {
        ui_msgbox "Git Not Found" "git is not installed.\n\nInstall with:\n  sudo apt install git"
        return 1
    }
    [[ -d "$REPO/.git" ]] || {
        ui_msgbox "Not a Git Repository" "No .git directory found under:\n$REPO"
        return 1
    }
    ...
    if [[ -z "$status_out" ]]; then
        ui_msgbox "GitHub Integration" \
            "Working tree is clean — nothing to commit.\n\nRepository : $GITHUB_REPO_NAME\nBranch     : $GITHUB_BRANCH"
        return 0
    fi

    git_offer_commit "Manual Commit"
}

# =============================================================================
# §G  QUICK BUILD
# =============================================================================
# One-click pipeline: environment check → build → detect ESP32 → flash →
# open monitor. Stops immediately if any step fails.
# =============================================================================

quick_build_ui() {
    if ! ui_yesno "Quick Build" \
        "Run Quick Build?\n\nThis performs, stopping immediately on any failure:\n\n\
1. Check environment\n\
2. Build firmware\n\
3. Detect ESP32\n\
4. Flash firmware\n\
5. Open serial monitor"; then
        return 0
    fi

    ui_wait_screen "Quick Build" "Step 1/5: Checking environment…"
    if ! env_verify 0; then
        ui_msgbox "Quick Build Stopped" \
            "Environment check FAILED.\n\nOpen 'Verify IDF Environment' from the main menu for details."
        return 1
    fi

    ui_wait_screen "Quick Build" "Step 2/5: Building firmware…"
    if ! build_firmware 1 1; then
        ui_msgbox "Quick Build Stopped" \
            "Build FAILED.\n\nCheck the build log (View Logs) for details."
        return 1
    fi

    ui_wait_screen "Quick Build" "Step 3/5: Detecting ESP32…"
    if ! serial_detect 1; then
        ui_msgbox "Quick Build Stopped" \
            "No ESP32 detected.\n\nConnect the board and try again."
        return 1
    fi

    ui_wait_screen "Quick Build" "Step 4/5: Flashing firmware…"
    if ! flash_firmware 1; then
        ui_msgbox "Quick Build Stopped" \
            "Flash FAILED.\n\nCheck the flash log (View Logs) for details."
        return 1
    fi

    ui_msgbox "Quick Build" "Step 5/5: Opening serial monitor…"
    serial_monitor_launch "$SELECTED_PORT"

    ui_msgbox "Quick Build Complete" \
        "✔  Environment OK → Built → Flashed → Monitor opened.\n\nBoard : $CFG_BOARD\nPort  : $SELECTED_PORT"

    git_offer_commit "Quick Build"
}
