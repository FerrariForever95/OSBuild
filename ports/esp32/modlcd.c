 ...
 *      from -1 to 21, and esp_lcd_panel_io_i80_config_t.cs_gpio_num now passes
 *      that real pin -- esp_lcd will drive CS itself per transaction from now
 *      on, instead of never touching it.
 *    - GPIO 21 was chosen (from the set of pins confirmed free on this board:
 *      3, 8, 19, 20, 21, 35, 36, 37, 45, 46, 47, 48) by eliminating: strapping
 *      pins (0, 3, 45, 46 -- affect boot mode, risky to toggle at runtime) and
 *      USB-JTAG pins (19, 20 -- also have a documented power-up glitch on
 *      ESP32-S3). GPIO 21 has no strapping, USB, or SPI-flash/PSRAM role on
 *      this chip, making it a clean, low-risk pick for a plain digital output.
 *    - This has NOT yet been tested on hardware -- per project rules, version
 *      stays dev/unverified until confirmed. The concrete risk this change
 *      introduces: esp_lcd's i80 CS timing (assert/deassert relative to
 *      DC/WR) is now live and must actually meet the ILI9488's CS setup/hold
 *      requirements, which were never exercised when CS was hard-wired low.
 *      If the display regresses after this change, CS timing is the first
 *      thing to suspect -- not the RAMWR/RAMWRC sequencing fixed in
 *      v1.4.0-dev/v1.5.0-dev, which is unrelated and unaffected by this change.
 *    - Updated the pin map header and the "always listening" design note
 *      (above moclcd_bus_pins_safe_idle()) to reflect that CS is no longer
 *      grounded; moclcd_bus_pins_safe_idle() itself is UNCHANGED -- it never
 *      touched CS (that pin is owned entirely by esp_lcd once the bus is
 *      created) and still doesn't, so this is a documentation-accuracy fix
 *      alongside the pin change, not a behavior change to that function.
 *    - No other functional changes from v1.5.0-dev. RAMWR/RAMWRC pixel
 *      streaming, panel init sequence, and all drawing primitives are
 *      untouched.
 *
 *  v1.5.0-dev
 ...
 * =================================================================================== */
#define MOCLCD_VERSION_STRING   "1.7.0-dev"
...
/*
 * IMPORTANT -- "the display is always listening" (HISTORICAL NOTE, v1.0.0-dev
 * through v1.5.0-dev; CS is now a real GPIO as of v1.6.0-dev, see below):
 * On the original PCB, CS was hard-wired to GND, so from the instant the panel
 * had power, every transition on D0-D7/DC/WR/RD was live and would be latched
 * by the panel the moment WR saw a rising edge -- there was no window where
